...
class TestCompare(unittest.TestCase):
    ...
    def test_user_compare_negation_with_resolver(self):
        ...
        """
        with (
            patch("linotp.lib.resolver.getResolverList", return_value={"ad1": {}}),
    ...
    def test_user_compare_attribute(self):
        ...
        class StubUser:
            ...
            def get_full_qualified_names(self):
                return [f"{self.login}@{self.realm}", self.login]

        def matches(user_def, login, realm, info):
            return AttributeCompare().compare(StubUser(login, realm, info), user_def)
        ...
        assert matches("#department == sales", "u", "r", {"department": "sales"})
        assert not matches("#department == sales", "u", "r", {"department": "eng"})

        # '!=' inequality
        assert matches("#department != sales", "u", "r", {"department": "eng"})
        assert not matches("#department != sales", "u", "r", {"department": "sales"})
        ...
        with patch("linotp.lib.resolver.getResolverList", return_value={"ad1": {}}):
            assert matches("ad1:#mobile ~= 123", "someone", "r", {"mobile": "49123"})
            assert not matches(
